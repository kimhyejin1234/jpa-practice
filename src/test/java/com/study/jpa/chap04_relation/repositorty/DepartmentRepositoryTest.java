...
class DepartmentRepositoryTest {
    ...
    //엔터티들을 관리하는 역할을 수행하는 클래스(영속성 컨텍스트 관리함.spring data jap 에서만 사용하는게 아님)
    //영속성 컨텍스의 내의 내욕들을  db  에 반영시키거나 비워내거나 수명을 관리할 수 있는 객체.
    @Autowired
    ...
    void testLazyEager() {
        //3번 사월을 조회하고 싶은데 굳이 부서정보는 필요 없다.
        //given
        Long id = 3L;
        ...
        System.out.println("getName = " + employee.getDepartment().getName());
        System.out.println("\n\n\n");
    }
    @Test
    @DisplayName("양방향 연관관계에서 연관데이터의 수정")
    void testChangeDept() {
        //1번 사원의 부서를 1번 -> 2번 부서로 변경해야 한다.
        //given
        ...
        //1번 -> 변경 감지(더티 체크) 후 변경된 내용을 db 에 즉시 반영하는 역할
//        entityManager.flush();//db 를 밀어내기
